...
export function BottomNavigation({ active, onChange }: { active: AppTab; onChange: (tab: AppTab) => void }) {
  const { t } = useLocale();
  return <nav className="bottom-navigation" aria-label={t("Navigazione principale")}>{items.map(({id,label,icon:Icon}) => <button type="button" key={id} className={`${active === id ? "is-active" : ""} ${id === "map" ? "is-map" : ""}`} aria-current={active === id ? "page" : undefined} onClick={() => onChange(id)}><span className="nav-icon"><Icon size={24} weight="regular" /></span><span>{t(label)}</span></button>)}</nav>;
}
