...
export function BottomNavigation({ active, onChange }: { active: AppTab; onChange: (tab: AppTab) => void }) {
  return <nav className="bottom-navigation" aria-label="Navigazione principale">{items.map(({id,label,icon:Icon}) => <button type="button" key={id} className={`${active === id ? "is-active" : ""} ${id === "map" ? "is-map" : ""}`} aria-current={active === id ? "page" : undefined} onClick={() => onChange(id)}><Icon size={24} weight={active === id || id === "map" ? "fill" : "regular"} /><span>{label}</span></button>)}</nav>;
}
