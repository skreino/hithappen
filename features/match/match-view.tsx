...
export function MatchView({ events, history, saved, ready, onOpen, onAction, onProfile }: { events: EventItem[]; history: Choice[]; saved: Set<string>; ready: boolean; onOpen: (event: EventItem) => void; onAction: (action: DeviceAction) => void; onProfile: () => void }) {
  ...
  const choose = (kind: Choice["kind"]) => { if (event && ready) { clearGesture(); setCelebration({ id: event.id, title: event.title, kind }); onAction({ type: "choose", id: event.id, kind }); } };
  ...
  return <main className="view match-view">{celebration && <Celebration key={`${celebration.id}-${celebration.kind}`} title={celebration.title} kind={celebration.kind} />}<div className="match-copy"><div><p>MATCH · {history.length + 1} / {events.length}</p><h1>{t("Ti va questa serata?")}</h1></div><button className="icon-control" aria-label={t("Annulla ultima scelta")} disabled={!history.length} onClick={() => { setCelebration(null); onAction({ type: "undo" }); }}><ArrowCounterClockwise size={23} /></button><span>{t("Scegli un evento, non una persona.")} · {t("Destra per salvare, sinistra per passare.")}</span></div><progress className="match-progress" value={history.length} max={events.length} aria-label={t("Eventi già valutati")} />
    <div className="swipe-stage"><article key={event.id} ref={card} className="swipe-card" onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={clearGesture} onLostPointerCapture={clearGesture}>
      <img src={event.image} alt={t(event.imageAlt ?? event.title)} width="900" height="1100" draggable={false} /><div className="swipe-card__scrim" />
