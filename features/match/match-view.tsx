...
import { HitFlame } from "@/components/ui/hit-flame";
import { useLocale } from "@/lib/i18n/locale-provider";
/* eslint-disable @next/next/no-img-element -- Demo event photography. */
import { useEffect, useRef, useState, type PointerEvent } from "react";
import { ArrowCounterClockwise, ArrowUpRight, BookmarkSimple, Fire, MapPin, X } from "@phosphor-icons/react";
...
export function MatchView({ events, history, saved, ready, onOpen, onAction, onProfile }: { events: EventItem[]; history: Choice[]; saved: Set<string>; ready: boolean; onOpen: (event: EventItem) => void; onAction: (action: DeviceAction) => void; onProfile: () => void }) {
  ...
  if (!event) {
    ...
    return <main className="view match-summary">{celebration && <Celebration key={`${celebration.id}-${celebration.kind}`} title={celebration.title} kind={celebration.kind} />}<div className="match-copy"><p>{t("SELEZIONE COMPLETATA")}</p><h1>{liked.length ? t("Hai trovato le tue serate.") : t("Nessuna scintilla, per ora.")}</h1><span>{liked.length} {t("eventi ti interessano.")} {liked.filter(item => saved.has(item.id)).length} {t("ancora nei salvati.")}</span></div><div className="editorial-list">{liked.map(item => <CompactEventRow key={item.id} event={item} onOpen={() => onOpen(item)} />)}</div><div className="match-summary__actions"><button className="secondary-cta" disabled={!history.length || !!exiting} onClick={() => { setCelebration(null); onAction({ type: "undo" }); }}>{t("Annulla ultima")}</button><button className="primary-cta" onClick={onProfile}>{t("Vedi salvati")}</button><button className="secondary-cta" onClick={() => { setCelebration(null); onAction({ type: "restart" }); }}>{t("Ricomincia")}</button></div><p className="demo-caption">{t("Match personale su eventi demo. Nessun abbinamento con altre persone.")}</p></main>;
  }
  return <main className="view match-view">{celebration && <Celebration key={`${celebration.id}-${celebration.kind}`} title={celebration.title} kind={celebration.kind} />}<div className="match-copy"><div><h1>Match</h1><p>{t("Scopri una serata alla volta.")}</p></div><div className="match-position"><span>{history.length + (exiting ? 0 : 1)} / {events.length}</span><button className="icon-control" aria-label={t("Annulla ultima scelta")} disabled={!history.length || !!exiting} onClick={() => { setCelebration(null); onAction({ type: "undo" }); }}><ArrowCounterClockwise size={23} /></button></div></div><progress className="match-progress sr-only" value={history.length} max={events.length} aria-label={t("Eventi già valutati")} />
    <p className="sr-only" id="match-instructions">{t("Destra per salvare, sinistra per passare.")}</p>
    <div className="swipe-stage"><article key={event.id} ref={card} aria-describedby="match-instructions" className={`swipe-card${exiting ? ` swipe-card--${exiting}` : ""}`} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={clearGesture} onLostPointerCapture={clearGesture}>
      <div className="match-photo"><img src={event.image} alt={t(event.imageAlt ?? event.title)} width="900" height="1100" draggable={false} fetchPriority="high" /><span className="swipe-stamp swipe-stamp--hit" aria-hidden="true"><Fire weight="fill" />HIT</span><span className="swipe-stamp swipe-stamp--nope" aria-hidden="true">NOPE</span>
      ...
      </div><div className="swipe-card__copy"><div className="match-category"><small>{t(event.category)}</small><button className="swipe-detail" disabled={!!exiting} onClick={() => onOpen(event)}>{t("Dettagli")}<ArrowUpRight size={16} aria-hidden="true" /></button></div><h2>{t(event.title)}</h2><p><MapPin size={17} />{event.neighborhood} · {event.distanceKm.toLocaleString(language === "en" ? "en-GB" : "it-IT")} km</p><footer><strong>{formatRelativeEventTime(event.startAt, undefined, language)}</strong><b>{formatPrice(event, language)}</b></footer></div>
    </article></div>
    <div className="swipe-actions"><button className="swipe-action" aria-label={t("Passa")} disabled={!!exiting} onClick={() => choose("pass")}><X size={25} /><span>NOPE</span></button><button className="swipe-action swipe-action--like" aria-label={t("Mi interessa")} disabled={!!exiting} onClick={() => choose("like")}><span className="hit-flame" aria-hidden="true"><HitFlame /></span><span>HIT</span></button></div><p className="demo-caption match-note">{t("Le scelte restano su questo dispositivo · demo")}</p>
  </main>;
}
