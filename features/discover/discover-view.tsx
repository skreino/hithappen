...
export function DiscoverView({ events, interests, saved, onOpen, onSave, onExplore }: { events: EventItem[]; interests: string[]; saved: Set<string>; onOpen: (event: EventItem) => void; onSave: (id: string) => void; onExplore: () => void }) {
  ...
  const go = (index: number) => {
    ...
    if (card) rail.current?.scrollTo({ left: card.offsetLeft, behavior: window.matchMedia("(prefers-reduced-motion: reduce)").matches ? "instant" : "smooth" });
  };
  ...
  return <main className="view discover-view"><div className="home-heading"><p>{t("STASERA A MILANO")}</p><h1>{t("La notte è ancora tua.")}</h1></div>
    ...
    <div className="carousel-controls"><span className="demo-caption">{featured[active] ? reason(featured[active]) : t("Vicinanza e interesse · demo")}</span><div className="carousel-dots">{featured.slice(0, Math.max(1, featured.length - 1)).map((event, index) => <button key={event.id} aria-label={`${t("Mostra serata")} ${index + 1}`} aria-pressed={active === index} onClick={() => go(index)}><i /></button>)}</div><button className="icon-control carousel-arrow" aria-label={t("Serata precedente")} disabled={active === 0} onClick={() => go(active - 1)}><ArrowLeft size={18} /></button><button className="icon-control carousel-arrow" aria-label={t("Serata successiva")} disabled={active >= featured.length - 2} onClick={() => go(active + 1)}><ArrowRight size={18} /></button></div></section>
